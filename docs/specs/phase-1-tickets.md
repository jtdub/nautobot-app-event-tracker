...
!!! warning "Draft — not yet approved"
    This is the execution spec for Phase 1. It was reconstructed from the project kickoff brief and the system diagram, and is pending review. Section 11 lists the calls made while writing it that most need a second opinion. Nothing in this spec has been implemented.
...
| `tickets/{id}/attach/` `tickets/{id}/detach/` | `POST` | Body carries `object_type` and `object_id` |
...
8. **UI.** The ticket detail page renders the grouped related-objects panel, the update timeline, and transition buttons for exactly the legal next states — no more — with buttons hidden from users lacking `transition_eventticket`. The navigation entries appear. The app contains no hand-written page template.
...
| `test_api.py` | Transition action success and permission denial; 409 mapping; `PATCH` with `status` rejected with 400; `ticket-updates/` rejects `POST`, `PATCH`, `DELETE`; creation routes through the service and produces a `created` update | 3, 6 |
...
| `test_views.py` | Standard viewset cases from the template's `ViewTestCases`; transition buttons render for exactly the legal set and vanish without the permission; the grouped panel groups by content type; the ticket form has no `status` field | 3, 8 |
...
These are the calls made while writing this spec that most deserve a second opinion. Each states a proposed reading so that silence can be taken as agreement.

**11.1 Attached objects have no dedicated model (section 3.4).** The brief names exactly three models, so attachments are derived from the update trail rather than stored in a join table. *Proposed reading:* keep it — attachment history comes out append-only for free, and the query is a single indexed filter over a ticket's updates. *Cost:* "currently attached" is a computation rather than a relation, so it cannot be filtered on with a plain join, and `related_object_type` in section 7 needs a subquery. If you would rather have a `TicketObject` through-model, this is the moment to say so — it changes sections 3.4, 4.2, 6 and 7.

**11.2 Status is a `ChoiceSet`, not a Nautobot `Status` (ADR 0002).** This departs from Nautobot convention, and a reviewer familiar with core will notice. *Proposed reading:* the departure is justified because the transition graph and the AI safety rule both key off specific states that must not be administrator-editable. Recorded as an ADR precisely because it is contestable.

**11.3 `SUPPRESSED` as a first-class state.** The brief does not name the states. I included `suppressed` because the Phase 2 pre-filter needs somewhere to put events judged to be noise while keeping them on the record. *Proposed reading:* keep it. If suppression should instead be a boolean flag or a tag, the graph loses two states and four edges.

**11.4 `resolved -> in_progress` is the only reopen path.** A ticket cannot be reopened once `closed`. *Proposed reading:* keep `closed` strictly terminal — it is what makes the AI immutability rule simple to state and to test. The cost is that a wrongly-closed ticket needs a new ticket rather than a reopen, which some operators will dislike.

**11.5 Rule S5 keys dedup on non-terminal status.** A recurrence while a ticket is still open joins that ticket; a recurrence after it resolves opens a new one. *Proposed reading:* as written. The alternative — a time window rather than a status test — is more tunable but needs a configuration knob this phase does not otherwise need.

**11.6 `TicketUpdate` is not a `PrimaryModel`.** It gets `BaseModel` and `ChangeLoggedModel` only. *Proposed reading:* correct for an audit row; tags and custom fields on a log entry would be strange, and change-logging an append-only record is circular. *Cost:* it needs a hand-written GraphQL type and a `BaseModelSerializer` rather than getting both for free.

**11.7 The MySQL scaffold artifacts.** ADR 0003 makes PostgreSQL the only supported backend, but the baked template still ships `invoke.mysql.yml`, `development/docker-compose.mysql.yml`, `development/development_mysql.env` and a MySQL leg in CI. *Proposed reading:* delete them as part of Phase 1, in their own commit, so the repository does not advertise support that does not exist. Not done yet, because deleting template files is a housekeeping decision rather than a spec one.

**11.8 Docker is unavailable in the current environment.** The definition of done calls for `invoke tests` to be green, but the template's invoke tasks drive `docker compose` and there is no Docker daemon here. *Proposed reading:* leave every invoke task untouched for use on a Docker-capable machine, and additionally verify Phase 1 by running the same `nautobot-server test` and `nautobot-server check` commands natively against a local PostgreSQL instance. The commands under test are identical; only the runner differs. Any claim that the suite is green will say which runner produced it.
