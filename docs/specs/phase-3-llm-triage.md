...
- **T5 — No model call inside a transaction, and a retry never pays twice.** `decide()` runs before `_write_ticket()`'s `transaction.atomic()`, so no transaction ever spans a network call. The runner's database retry (I5) re-enters `handle_message` for the same message; a one-entry memo keyed on `BrokerMessage.identity` returns the decision already paid for, and says it came from the memo so that `triaged` counts calls rather than attempts. Identity is `(topic, partition, offset)` where the broker numbers messages — Kafka counts offsets per partition, so neither half names a message alone — and `(topic, payload digest)` where it does not, which is what makes the memo work on Redis pub/sub too.
...
- `TriageFilter.decide()` takes the message identity as an explicit argument for the T5 memo, rather than a message object: the pipeline hands it the already-normalized event, which carries nothing of the broker.
- Startup validation resolves the litellm client as well as the model. The missing-extra refusal is an `ImproperlyConfigured`, deliberately outside the `LLMError` family that T4 fails open on, so nothing on the ingestion path would catch it: without the startup check the consumer starts cleanly and dies on its first accepted event.
- The usage-record link runs after the ticket commits but inside the runner's retry loop, so it swallows a database failure rather than raising. Raising would re-run a message whose ticket already exists, and for an event with no dedup key that means a second ticket.
