...
- **The consumer reconnects to a broker that goes away**, with backoff doubling to a minute and no limit, as section 5.2 requires. `EventConsumer.reconnect()` owns how; the loop owns when and how long to wait, so a second broker does not bring a second policy.
- **The message is counted as received by the loop, not the pipeline.** A retry after a database failure re-enters `handle_message()`, and counting it twice would break the invariant in section 4.1.
