...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_EVENT_TRACKER_xxx
...
def docker_compose(context, command, **kwargs):
    ...
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_event_tracker.project_name}",
    ...
    if (
        context.nautobot_event_tracker.ephemeral_ports
        and context.nautobot_event_tracker.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_event_tracker.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_event_tracker.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_event_tracker.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.nautobot_event_tracker.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_event_tracker.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_event_tracker.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_event_tracker.python_ver
    dockerfile_path = os.path.join(context.nautobot_event_tracker.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_event_tracker.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_event_tracker"
...
@task(
    help={
        "version": "Version of Event Tracker to generate the release notes for.",
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_event_tracker'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_event_tracker")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_event_tracker.migrations"
    ...
    if migrations_dir.is_dir():
        if run_migrations_check:
            migrations_pylint_command = (
                ...
                " nautobot_event_tracker.migrations"
            )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_event_tracker/templates/"
...
def tests(context, failfast=False, keepdb=False, no_input=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_event_tracker.local):
